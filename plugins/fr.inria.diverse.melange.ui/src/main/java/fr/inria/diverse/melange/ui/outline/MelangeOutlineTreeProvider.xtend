...
class MelangeOutlineTreeProvider extends DefaultOutlineTreeProvider {
	...
	def void _createChildren(IOutlineNode parentNode, ModelTypingSpace root) {
		...
		val externalLangs = root.languages.map[allDependencies].flatten.filter[it.eContainer !== root].toSet
		externalLangs.forEach [ lang |
			val implements = if(lang instanceof Language) lang.implements else newArrayList()
			val img = imageHelper.getImage("link.png")
			val txt = '''«lang.name»«FOR t : lang.operators.filter(Inheritance).map[targetLanguage] BEFORE '\u25C0' SEPARATOR ', '»«
			»«t.name»«ENDFOR»«FOR t : implements BEFORE ' \u25C1 ' SEPARATOR ', '»«t.name»«ENDFOR»'''.toString
